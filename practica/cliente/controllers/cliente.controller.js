...
exports.crearPedido = async (req, res) => {
  ...
  console.log({
    entidad: "Cliente",
    message:
      "Pedido Enviado a Restaurante ID: " +
      pedido.id +
      " Total: " +
      pedido.total_pedido,
  });
  await axios.post(`http://localhost:3004/esb/restaurante`, {
...
exports.obtenerPedidoRestaurante = async (req, res) => {
  const { id } = req.params;
  const respuestaRestaurante = await axios.post(
    `http://localhost:3004/esb/restaurante`,
    { operation: "getOrder", data: { id } }
  );
...
exports.obtenerPedidoRepartidor = async (req, res) => {
  const { id } = req.params;
  const respuestaRepartidor = await axios.post(
    `http://localhost:3004/esb/repartidor`,
    { operation: "getOrder", data: { id } }
  );
